...
#ifndef NERTC_ENGINE_DEFINES_H_
#define NERTC_ENGINE_DEFINES_H_
...
/** 接口ID类型。*/
enum NERtcInterfaceIdType {
    kNERtcIIDAudioDeviceManager = 1, /**< 获取音频设备管理器的接口ID */
    kNERtcIIDVideoDeviceManager = 2, /**< 获取视频设备管理器的接口ID */
};

/** 参会者角色类型 */
enum NERtcClientRole {
    kNERtcClientRoleBroadcaster =
        0, /**< （默认）直播模式中的主播，可以操作摄像头等音视频设备、发布流、配置互动直播推流任务、上下线对房间内其他用户可见。
            */
    kNERtcClientRoleAudience =
        1, /**< 直播模式中的观众，观众只能接收音视频流，不支持操作音视频设备、配置互动直播推流任务、上下线不通知其他用户。 */
};
...
struct NERtcLiveStreamTaskOption {
    ...
    char layout_parameters[kNERtcMaxBuffLength]; /**< 自定义布局参数（选填），JSON 字符串格式, 只有当layout_mode为
                                                kNERtcLayoutCustom 或 kNERtcLayoutAudioOnly时才需要设置。设置详见:
                                                http://netease.im/blog/12-3 */
};

/** 直播推流模式 */
enum NERtcLiveStreamMode {
...
/** 视频帧数据回调
    ...
    @param  task_id  任务id
    @param  url 推流地址
    @param  error_code 错误码，参考NERtcErrorCode
...
#ifdef __APPLE__
typedef std::function<void(const char *task_id, const char *url, int error_code, void *opaque)> onNERtcLiveStreamTaskCallback;
#else

typedef void (*onNERtcLiveStreamTaskCallback)(const char *task_id, const char *url, int error_code, void *opaque);
...
struct NERtcLiveStreamTaskInfo {
    ...
    /**
     直播推流模式。默认为 。详细信息请参考 NERtcLiveStreamMode。
     */
...
enum NERtcAudioProfileType {
    kNERtcAudioProfileDefault =
        0, /**< 0: 默认设置。Speech场景下为 kNERtcAudioProfileStandardExtend，Music场景下为 kNERtcAudioProfileHighQuality */
...
/** 视频流类型。*/
enum NERtcRemoteVideoStreamType {
...
/** 耳机补偿类型 */
typedef enum {
    kNERtcHpCompOff = 0,					/**< 默认关闭 */
    kNERtcHpCompAkgK272 = 1,				/**< AKG K272 */
    kNERtcHpCompBoseSoundSport = 2,			/**< Bose Sound Sport */
    kNERtcHpCompSennheiserHd280pro = 3,		/**< Sennheiser HD 280 PRO */
    kNERtcHpCompSennheiser598Se = 4,		/**< Sennheiser 598SE */
} NERtcHpCompType;

/** 摄像头设备链接类型。*/
enum NERtcVideoDeviceTransportType {
...
/** 视频设备类型。*/
enum NERtcVideoDeviceType {
    kNERtcVideoDeviceUnknown = 0, /**< 未知视频设备 */
    kNERtcVideoDeviceCapture,     /**< 视频采集设备 */
};

/** 视频设备类型状态。*/
enum NERtcVideoDeviceState {
    kNERtcVideoDeviceAdded = 0,         /**< 视频设备已添加 */
    kNERtcVideoDeviceRemoved = 1,       /**< 视频设备已拔除 */
    kNERtcVideoDeviceInitialized = 2,   /**< 视频设备已初始化 */
    kNERtcVideoDeviceStarted = 3,       /**< 视频设备已打开 */
    kNERtcVideoDeviceStoped = 4,        /**< 视频设备已停止 */
    kNERtcVideoDeviceUnInitialized = 5, /**< 视频设备已释放 */
};

/** @enum NERtcVideoScalingMode 设置视频缩放模式。*/
enum NERtcVideoScalingMode {
    kNERtcVideoScaleFit =
        0,                        /**<
...
enum NERtcVideoMirrorMode {
    kNERtcVideoMirrorModeAuto = 0, /**< 0: Windows/macOS SDK 启用镜像模式。在 iOS/Android 平台中：如果你使用前置摄像头，SDK
                                  默认启用镜像模式；如果你使用后置摄像头，SDK 默认关闭镜像模式。*/
...
/** Android Camera类型*/
enum NERtcCameraType {
    kNERtcCameraTypeUnknown = 0,
    kNERtcCameraType1 = 1,
    kNERtcCameraType2 = 2,
};

/** Android 视频数据颜色空间格式 */
enum NERtcVideoColorFormat {
    NERtcVideoColorTexture = 0,
    NERtcVideoColorI420 = 1,
    NERtcVideoColorNV21 = 2,
};
...
struct NERtcVideoConfig {
    ...
    /**
     * Android Camera类型，仅Android有效
     */
    NERtcCameraType cameraType;
    /**
     * 是否默认打开前置摄像头，仅Android有效
     */
    bool frontCamera;
    /**
     * 视频采集颜色空间格式, 默认为Texture，仅Android有效
     */
    NERtcVideoColorFormat colorFormat;
};
...
*/
#ifdef __APPLE__
typedef std::function<void(uid_t uid,          /**< uid */
                           void *data,         /**< 数据指针 */
...
typedef void (*onFrameDataCallback)(uid_t uid,          /**< uid */
                                    void *data,         /**< 数据指针 */
                                    uint32_t type,      /**< NERtcVideoType */
                                    uint32_t width,     /**< 宽度 */
                                    uint32_t height,    /**< 高度 */
...
enum NERtcScreenProfileType {
    kNERtcScreenProfile240P = 0,    /**< 320x240, 5fps */
    kNERtcScreenProfile480P = 1,    /**< 640x480, 5fps */
    kNERtcScreenProfileHD720P = 2,  /**< 1280x720, 5fps */
    kNERtcScreenProfileHD1080P = 3, /**< 1920x1080, 5fps。默认 */
    kNERtcScreenProfileCustom = 4,  /**< 自定义 */
...
enum NERtcSubStreamContentPrefer {
...
struct NERtcScreenCaptureParameters {
    ...
    bool capture_mouse_cursor; /**< 是否采集鼠标用于屏幕共享。iOS/Android无效*/
    bool window_focus; /**< 调用 startScreenCaptureByWindowId 方法共享窗口时，是否将该窗口前置。iOS/Android无效*/
    void *excluded_window_list;         /**< 待屏蔽窗口的 ID 列表。iOS/Android无效 */
    int excluded_window_count;          /**< 待屏蔽窗口的数量。iOS/Android无效*/
    NERtcSubStreamContentPrefer prefer; /**< 编码策略倾向。*/
};

/** 视频显示设置 */
struct NERtcVideoCanvas {
    bool use_external_video_render; /**< 是否使用外置渲染。*/
    onFrameDataCallback cb; /**< 数据回调  如果是macosx，需要设置NERtcEngineContext的video_use_exnternal_render为true才有效*/
    void *user_data; /**< 数据回调的用户透传数据
                    如果是macosx，需要设置NERtcEngineContext的video_use_exnternal_render为true才有效 */
...
/** 音频帧请求数据的读写模式。*/
enum NERtcRawAudioFrameOpModeType {
...
enum NERtcAudioOutputRouting {
    kNERtcAudioOutputRoutingDefault = 0, /**< 系统默认。*/
    kNERtcAudioOutputRoutingHeadset,     /**< 耳机。*/
    kNERtcAudioOutputRoutingEarpiece,    /**< 听筒。*/
    kNERtcAudioOutputRoutingLoudspeaker, /**< 扬声器。*/
    kNERtcAudioOutputRoutingBluetooth,   /**< 蓝牙外设。*/
};

/**
音频会话控制权限SDK 对 Audio Session 的控制权限*/
enum NERtcAudioSessionOperationRestriction {
    kNERtcAudioSessionOperationRestrictionNone = 0, /**< 没有限制，SDK 可以完全控制 Audio Session 操作。*/
    kNERtcAudioSessionOperationRestrictionAll, /**< 限制 SDK 对 Audio Session 进行任何操作，SDK 将不能再对 Audio Session
                                              进行任何配置。*/
};
...
class INERtcAudioFrameObserver {
   public:
    ...
    /** 获取单个远端用户混音前的音频数据。
     成功注册音频观测器后，如果订阅了远端音频（默认订阅）且远端用户开启音频后，SDK
...
enum NERtcVideoType {
    ...
    kNERtcVideoTypeRGBA = 4,          /**< RGBA 视频格式。*/
    kNERtcVideoTypeCVPixelBuffer = 5, /**< oc capture native视频格式.*/
    kNERtcVideoTypeTextureOES = 6,    /**< Android texture oes视频格式. */
    kNERtcVideoTypeTextureRGB = 7,    /**< Android texture rgb视频格式. */
};

/** 视频旋转角度。*/
enum NERtcVideoRotation {
...
////////////////////////////////////////////////////////
/** \addtogroup ParameterKeys
通过 JSON 配置 SDK 提供技术预览或特别定制功能。以标准化方式公开 JSON 选项。详见API \ref nertc::IRtcEngineEx::setParameters
"setParameters"
 @{
 */

////////////////////////////////////////////////////////
/**< bool value 优先使用硬件编码视频数据。默认值 true，该值建议在调用初始化接口：setupEngineWithContext
 * 之前设置，否则需要在下次初始化之后才生效 (仅iOS/Mac/android有效）*/
#define kNERtcKeyVideoPreferHWEncode "kNERtcKeyVideoPreferHWEncode"

/**< bool value 优先使用硬件解码视频数据。默认值 YES，该值建议在调用初始化接口：setupEngineWithContext
 * 之前设置，否则需要在下次初始化之后才生效(仅iOS/Mac/android有效） */
#define kNERtcKeyVideoPreferHWDecode "kNERtcKeyVideoPreferHWDecode"

/**< bool value 是否需要开启视频数据采集回调，从而开发者可以获取到原始视频数据。默认值 iOS/Android 为false，其他平台默认值为true
 */
#define kNERtcKeyVideoCaptureObserverEnabled "kNERtcKeyVideoCaptureObserverEnabled"

/**< bool value 是否优先使用 Metal 渲染，否则使用 OpenGL 渲染。默认值 true.(仅iOS有效）*/
#define kNERtcKeyVideoPreferMetalRender "kNERtcKeyVideoPreferMetalRender"

/**< bool value. true: 第一次开启摄像头时，使用后摄像头。默认值为false。（仅iOS/Android有效）  */
#define kNERtcKeyVideoStartWithBackCamera "kNERtcKeyVideoStartWithBackCamera"

/**< bool value. true: 录制主讲人, false: 不是录制主讲人。通话前设置有效 */
#define kNERtcKeyRecordHostEnabled "kNERtcKeyRecordHostEnabled"

/**< bool value，启用服务器音频录制。默认值 false。通话前设置有效 */
#define kNERtcKeyRecordAudioEnabled "kNERtcKeyRecordAudioEnabled"

/**< bool value，启用服务器视频录制。默认值 false。通话前设置有效 */
#define kNERtcKeyRecordVideoEnabled "kNERtcKeyRecordVideoEnabled"

/**< int value, NERtcRecordType。通话前设置有效 */
#define kNERtcKeyRecordType "kNERtcKeyRecordType"

/**< bool value，其他用户打开音频时，自动订阅。默认值 true。通话前设置有效 */
#define kNERtcKeyAutoSubscribeAudio "kNERtcKeyAutoSubscribeAudio"

/**< bool value，开启旁路直播。默认值 false。通话前设置有效 */
#define kNERtcKeyPublishSelfStreamEnabled "kNERtcKeyPublishSelfStreamEnabled"

/**< int value, NERtcLogLevel，SDK 输出小于或等于该级别的log，默认为 kNERtcLogLevelInfo */
#define kNERtcKeyLogLevel "kNERtcKeyLogLevel"

/**< bool value. AEC开关，默认值 true */
#define kNERtcKeyAudioProcessingAECEnable "kNERtcKeyAudioProcessingAECEnable"

/**< bool value. low level AEC开关，默认值 false,需要kNERtcKeyAudioProcessingAECEnable打开才生效 */
#define kNERtcKeyAudioAECLowLevelEnable "kNERtcKeyAudioAECLowLevelEnable"

/**< bool value. AGC开关，默认值 true */
#define kNERtcKeyAudioProcessingAGCEnable "kNERtcKeyAudioProcessingAGCEnable"

/**< bool value. NS开关，默认值 true */
#define kNERtcKeyAudioProcessingNSEnable "kNERtcKeyAudioProcessingNSEnable"

/**< bool value.AI  NS开关，建议通话前修改，默认值 false */
#define kNERtcKeyAudioProcessingAINSEnable "kNERtcKeyAudioProcessingAINSEnable"

/**< bool value. 输入混音开关，默认值 false */
#define kNERtcKeyAudioProcessingExternalAudioMixEnable "kNERtcKeyAudioProcessingExternalAudioMixEnable"

/**< bool value. 通知SDK是否使用耳机， true: 使用耳机, false: 不使用耳机，默认值 false */
#define kNERtcKeyAudioProcessingEarphone "kNERtcKeyAudioProcessingEarphone"

/**< int value. NERtcSendOnPubType；设置视频发送策略，默认发送大流 kNERtcSendOnPubHigh。通话前设置有效 */
#define kNERtcKeyVideoSendOnPubType "kNERtcKeyVideoSendOnPubType"

/**< bool value. 系统切换听筒事件时，禁用切换到扬声器。默认值 false。 如设置true
 * 则禁止SDK在系统切换到听筒时做切换扬声器操作，需要用户自己处理切换听筒事件(仅iOS有效) */
#define kNERtcKeyDisableOverrideSpeakerOnReceiver "kNERtcKeyDisableOverrideSpeakerOnReceiver"

/**< bool value. 设置耳机时不使用软件回声消除功能，默认值 false。如设置true
 * 则SDK在耳机模式下不使用软件回声消除功能，会对某些机型下 耳机的音质效果有影响(仅iOS有效)  */
#define kNERtcKeyDisableSWAECOnHeadset "kNERtcKeyDisableSWAECOnHeadset"

/**< bool value. 是否关闭蓝牙sco，默认值 false */
#define kNERtcKeyBluetoothSCO "kNERtcKeyBluetoothSCO"

/**< bool value. 设置视频数据采集回调时候，是否需要i420 yuv数据 */
#define kNERtcKeyCallbackCapturedVideoFrameTextureWithI420 "kNERtcKeyCallbackCapturedVideoFrameTextureWithI420"

#define kNERtcKeyCameraType "kNERtcKeyCameraType"
